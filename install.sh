...
mkdir -p "$INSTALL_DIR"/{logs,backup,config}; cp update-ddns.sh telegram.sh logger.sh cloudflare-api.sh utils.sh daily-status.sh notify-failure.sh upgrade.sh "$INSTALL_DIR/"; chmod +x "$INSTALL_DIR"/*.sh
...
cp systemd/cloudflare-ddns.service /etc/systemd/system/cloudflare-ddns.service; cp systemd/cloudflare-ddns.timer /etc/systemd/system/cloudflare-ddns.timer; cp systemd/cloudflare-ddns-status.service /etc/systemd/system/cloudflare-ddns-status.service; cp systemd/cloudflare-ddns-status.timer /etc/systemd/system/cloudflare-ddns-status.timer; cp systemd/cloudflare-ddns-failure@.service /etc/systemd/system/cloudflare-ddns-failure@.service; cp systemd/cloudflare-ddns.logrotate /etc/logrotate.d/cloudflare-ddns-telegram; sed -i "s|__INSTALL_DIR__|$INSTALL_DIR|g" /etc/systemd/system/cloudflare-ddns.service /etc/systemd/system/cloudflare-ddns-status.service /etc/systemd/system/cloudflare-ddns-failure@.service; sed -i "s|OnUnitActiveSec=.*|OnUnitActiveSec=${INTERVAL}min|g" /etc/systemd/system/cloudflare-ddns.timer; h="${DAILY_STATUS_TIME%%:*}"; m="${DAILY_STATUS_TIME##*:}"; sed -i "s|OnCalendar=.*|OnCalendar=*-*-* ${h}:${m}:00|g" /etc/systemd/system/cloudflare-ddns-status.timer; systemctl daemon-reload; systemctl enable --now cloudflare-ddns.timer; [[ "$DAILY_STATUS_ENABLED" == yes ]] && systemctl enable --now cloudflare-ddns-status.timer; "$INSTALL_DIR/update-ddns.sh" --startup || true; echo "Installation complete."
